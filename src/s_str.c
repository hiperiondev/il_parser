 ...
 */
static inline s_str s_str_create_temp(const s_str *const this_,
		const size_t num_elem) {
	if (num_elem >= access_struct(*this_).size) {
		s_str temp = malloc(
				sizeof(struct S_STRING_STRUCT)
						+ sizeof(char) * (size_t) (1.5 * num_elem));
...
s_str s_str_create_from_multi_char(const size_t length, const char c) {
	s_str new_str = malloc(
			sizeof(struct S_STRING_STRUCT) + sizeof(char) * (length + 1));
...
s_str s_str_create_from_s_str(const s_str *const s_str_ptr_for_create) {
	...
	if (s_str_ptr_for_create == NULL) {
		...
		new_str = malloc(
				sizeof(struct S_STRING_STRUCT)
						+ sizeof(char)
								* access_struct(*s_str_ptr_for_create).size);

		if (new_str != NULL) {
			access_struct(new_str).capacity =
					access_struct(*s_str_ptr_for_create).size;
			access_struct(new_str).size =
					access_struct(*s_str_ptr_for_create).size;

			memcpy(access_struct(new_str).string,
					access_struct(*s_str_ptr_for_create).string,
					sizeof(char) * access_struct(*s_str_ptr_for_create).size);
		}
	}
...
void s_str_assign_multi_char(s_str *const this_, const size_t length,
		const char c) {
	assert(this_ != NULL && *this_ != NULL);
...
void s_str_assign_c_str(s_str *const this_, const char *const c_str_ptr) {
	...
	if (access_struct(*this_).capacity < strlen(c_str_ptr) + 1) {
		...
		if (temp != NULL) {
			memcpy(access_struct(temp).string, c_str_ptr,
					sizeof(char) * (strlen(c_str_ptr) + 1));
		...
		memcpy(access_struct(*this_).string, c_str_ptr,
				sizeof(char) * (strlen(c_str_ptr) + 1));
...
void s_str_assign_s_str(s_str *const this_,
		const s_str *const s_str_ptr_for_assign) {
	assert(
			this_ != NULL && *this_ != NULL && s_str_ptr_for_assign != NULL && *s_str_ptr_for_assign != NULL);

	if (access_struct(*this_).capacity
			< access_struct(*s_str_ptr_for_assign).size) {
		s_str temp = s_str_create_temp(this_,
				access_struct(*s_str_ptr_for_assign).size);

		if (temp != NULL) {
			memcpy(access_struct(temp).string,
					access_struct(*s_str_ptr_for_assign).string,
					sizeof(char) * access_struct(*s_str_ptr_for_assign).size);

			free(*this_);
			*this_ = temp;
		}
	} else {
		memcpy(access_struct(*this_).string,
				access_struct(*s_str_ptr_for_assign).string,
				sizeof(char) * access_struct(*s_str_ptr_for_assign).size);
...
void s_str_remove_at(const s_str *const this_, const size_t index) {
	...
	if (index < access_struct(*this_).size - 1) {
		memmove(access_struct(*this_).string + index,
				access_struct(*this_).string + index + 1,
				sizeof(char) * (access_struct(*this_).size - (index + 1)));
...
size_t s_str_find_c_str(const s_str *const this_, const char *const c_str_ptr) {
	...
	if (strlen(c_str_ptr) == 0) {
		...
		for (size_t counter = 0;
				counter < access_struct(*this_).size - strlen(c_str_ptr);
				++counter) {
			if (memcmp(access_struct(*this_).string + counter, c_str_ptr,
					sizeof(char) * strlen(c_str_ptr)) == 0) {
				return counter;
			}
		}
	}
...
size_t s_str_find_s_str(const s_str *const this_,
		const s_str *const s_str_ptr_for_find) {
	assert(
			this_ != NULL && *this_ != NULL && s_str_ptr_for_find != NULL && *s_str_ptr_for_find != NULL);

	if (access_struct(*s_str_ptr_for_find).size == 1) {
		return s_str_find_char(this_, '\0');
	} else if (access_struct(*s_str_ptr_for_find).size - 1
			< access_struct(*this_).size) {
		for (size_t counter = 0;
				counter
						< access_struct(*this_).size
								- (access_struct(*s_str_ptr_for_find).size - 1);
				++counter) {
			if (memcmp(access_struct(*this_).string + counter,
					access_struct(*s_str_ptr_for_find).string,
					sizeof(char)
							* (access_struct(*s_str_ptr_for_find).size - 1))
					== 0) {
...
size_t s_str_rfind_c_str(const s_str *const this_, const char *const c_str_ptr) {
	...
	if (strlen(c_str_ptr) == 0) {
		...
		for (size_t counter = access_struct(*this_).size - 1
				- strlen(c_str_ptr);; --counter) {
			if (memcmp(access_struct(*this_).string + counter, c_str_ptr,
					sizeof(char) * strlen(c_str_ptr)) == 0) {
				return counter;
			} else if (counter == 0) {
...
size_t s_str_rfind_s_str(const s_str *const this_,
		const s_str *const s_str_ptr_for_rfind) {
	assert(
			this_ != NULL && *this_ != NULL && s_str_ptr_for_rfind != NULL && *s_str_ptr_for_rfind != NULL);

	if (access_struct(*s_str_ptr_for_rfind).size == 1) {
		return s_str_rfind_char(this_, '\0');
	} else if (access_struct(*s_str_ptr_for_rfind).size - 1
			< access_struct(*this_).size) {
		for (size_t counter = access_struct(*this_).size
				- access_struct(*s_str_ptr_for_rfind).size;; --counter) {
			if (memcmp(access_struct(*this_).string + counter,
					access_struct(*s_str_ptr_for_rfind).string,
					sizeof(char)
							* (access_struct(*s_str_ptr_for_rfind).size - 1))
					== 0) {
...
void s_str_reserve(s_str *const this_, const size_t num_elem) {
	...
	if (num_elem > access_struct(*this_).size) {
		s_str temp = realloc(*this_,
				sizeof(struct S_STRING_STRUCT) + sizeof(char) * num_elem);
...
void s_str_trim(const s_str *const this_) {
	...
	while (access_struct(*this_).size > 1
...
void s_str_shrink_to_fit(s_str *const this_) {
	...
	if (access_struct(*this_).capacity != access_struct(*this_).size) {
		s_str temp = realloc(*this_,
				sizeof(struct S_STRING_STRUCT)
						+ sizeof(char) * access_struct(*this_).size);
...
void s_str_push_back(s_str *const this_, const char c) {
	...
	if (access_struct(*this_).size + 1 > access_struct(*this_).capacity) {
		...
		if (temp != NULL) {
			memcpy(access_struct(temp).string, access_struct(*this_).string,
...
void s_str_append_c_str(s_str *const this_, const char *const c_str_ptr) {
	...
	if (access_struct(*this_).size + strlen(c_str_ptr)
			> access_struct(*this_).capacity) {
		s_str temp = s_str_create_temp(this_,
		...
		if (temp != NULL) {
			memcpy(access_struct(temp).string, access_struct(*this_).string,
					sizeof(char) * (access_struct(*this_).size - 1));
			memcpy(access_struct(temp).string + access_struct(*this_).size - 1,
					c_str_ptr, sizeof(char) * (strlen(c_str_ptr) + 1));

			free(*this_);
			*this_ = temp;
		}
	} else {
		memcpy(access_struct(*this_).string + access_struct(*this_).size - 1,
				c_str_ptr, sizeof(char) * (strlen(c_str_ptr) + 1));
...
void s_str_append_s_str(s_str *const this_,
		const s_str *const s_str_ptr_for_append) {
	assert(
			this_ != NULL && *this_ != NULL && s_str_ptr_for_append != NULL && *s_str_ptr_for_append != NULL);

	if (access_struct(*this_).size + access_struct(*s_str_ptr_for_append).size
			> access_struct(*this_).capacity) {
		s_str temp = s_str_create_temp(this_,
				access_struct(*this_).size
						+ access_struct(*s_str_ptr_for_append).size - 1);

		if (temp != NULL) {
			memcpy(access_struct(temp).string, access_struct(*this_).string,
					sizeof(char) * (access_struct(*this_).size - 1));
			memcpy(access_struct(temp).string + access_struct(*this_).size - 1,
					access_struct(*s_str_ptr_for_append).string,
		...
		access_struct(*this_).size += access_struct(*s_str_ptr_for_append).size
				- 1;
	}
}
...
void s_str_remove_first_word(const s_str *const this_) {
	...
	if (access_struct(*this_).size > 1) {
		...
		if (index == access_struct(*this_).size - 1) {
			...
			memmove(access_struct(*this_).string,
					access_struct(*this_).string + index + 1,
					sizeof(char) * (access_struct(*this_).size - (index + 1)));
...
void s_str_erase(const s_str *const this_, const size_t index, size_t length) {
	...
	if (index < access_struct(*this_).size - 1) {
		...
		memmove(access_struct(*this_).string + index,
				access_struct(*this_).string + index + length,
	...
	if (index < access_struct(*this_).size) {
		if (access_struct(*this_).size + 1 > access_struct(*this_).capacity) {
			...
			if (temp != NULL) {
				memcpy(access_struct(temp).string, access_struct(*this_).string,
						sizeof(char) * index);
				memcpy(access_struct(temp).string + index + 1,
						access_struct(*this_).string + index,
			...
			memmove(access_struct(*this_).string + index + 1,
					access_struct(*this_).string + index,
					sizeof(char) * (access_struct(*this_).size - index));
...
void s_str_insert_c_str(s_str *const this_, const size_t index,
		const char *const c_str_ptr) {
	assert(this_ != NULL && *this_ != NULL && c_str_ptr != NULL);
	...
	if (index < access_struct(*this_).size && strlen(c_str_ptr) > 0) {
		if (access_struct(*this_).size + strlen(c_str_ptr)
			...
			if (temp != NULL) {
				memcpy(access_struct(temp).string, access_struct(*this_).string,
						sizeof(char) * index);
				memcpy(access_struct(temp).string + index + strlen(c_str_ptr),
						access_struct(*this_).string + index,
						sizeof(char) * (access_struct(*this_).size - index));
				memcpy(access_struct(temp).string + index, c_str_ptr,
						sizeof(char) * strlen(c_str_ptr));

				free(*this_);
				*this_ = temp;
			}
		} else {
			memmove(access_struct(*this_).string + index + strlen(c_str_ptr),
					access_struct(*this_).string + index,
					sizeof(char) * (access_struct(*this_).size - index));
			memcpy(access_struct(*this_).string + index, c_str_ptr,
					sizeof(char) * strlen(c_str_ptr));
...
void s_str_insert_s_str(s_str *const this_, const size_t index,
		const s_str *const s_str_ptr_for_insert) {
	assert(
			this_ != NULL && *this_ != NULL && s_str_ptr_for_insert != NULL && *s_str_ptr_for_insert != NULL);

	if (index < access_struct(*this_).size
			&& access_struct(*s_str_ptr_for_insert).size - 1 > 0) {
		if (access_struct(*this_).size
				+ access_struct(*s_str_ptr_for_insert).size
				> access_struct(*this_).capacity) {
			s_str temp = s_str_create_temp(this_,
					access_struct(*this_).size
							+ access_struct(*s_str_ptr_for_insert).size - 1);

			if (temp != NULL) {
				memcpy(access_struct(temp).string, access_struct(*this_).string,
						sizeof(char) * index);
				memcpy(
						access_struct(temp).string + index
								+ access_struct(*s_str_ptr_for_insert).size - 1,
						access_struct(*this_).string + index,
						sizeof(char) * (access_struct(*this_).size - index));
				memcpy(access_struct(temp).string + index,
						access_struct(*s_str_ptr_for_insert).string,
						sizeof(char)
								* (access_struct(*s_str_ptr_for_insert).size - 1));
			...
			memmove(
					...
					access_struct(*this_).string + index,
					sizeof(char) * (access_struct(*this_).size - index));
			memcpy(access_struct(*this_).string + index,
					access_struct(*s_str_ptr_for_insert).string,
					sizeof(char)
							* (access_struct(*s_str_ptr_for_insert).size - 1));
...
void s_str_sepwith_char(s_str *const this_, const char c) {
	...
	if (access_struct(*this_).size > 1) {
		char *const slate = malloc(
				sizeof(char)
						* (access_struct(*this_).size
								+ (access_struct(*this_).size - 2) * 1));

		if (slate != NULL) {
			for (size_t num_iterations = 0, counter = 0;
					num_iterations < access_struct(*this_).size - 2;
					++num_iterations, counter += 2) {
				slate[counter] = access_struct(*this_).string[num_iterations];

				slate[counter + 1] = c;
			}
			slate[access_struct(*this_).size
					+ (access_struct(*this_).size - 2) * 1 - 2] =
					access_struct(*this_).string[access_struct(*this_).size - 2];
			slate[access_struct(*this_).size
					+ (access_struct(*this_).size - 2) * 1 - 1] = '\0';
			...
			if (access_struct(*this_).size
				...
				access_struct(*this_).size += (access_struct(*this_).size - 2)
						* 1;
			}
...
void s_str_sepwith_c_str(s_str *const this_, const char *const c_str_ptr) {
	...
	if (access_struct(*this_).size > 1 && strlen(c_str_ptr) > 0) {
		...
		if (slate != NULL) {
			for (size_t num_iterations = 0, counter = 0;
					num_iterations < access_struct(*this_).size - 2;
					++num_iterations, counter += 1 + strlen(c_str_ptr)) {
				slate[counter] = access_struct(*this_).string[num_iterations];
				memcpy(slate + counter + 1, c_str_ptr,
						sizeof(char) * strlen(c_str_ptr));
			}
			slate[access_struct(*this_).size
					+ (access_struct(*this_).size - 2) * strlen(c_str_ptr) - 2] =
					access_struct(*this_).string[access_struct(*this_).size - 2];
			slate[access_struct(*this_).size
					+ (access_struct(*this_).size - 2) * strlen(c_str_ptr) - 1] =
					'\0';

			if (access_struct(*this_).size
					+ (access_struct(*this_).size - 2) * strlen(c_str_ptr)
					> access_struct(*this_).capacity) {
				s_str temp = s_str_create_temp(this_,
						access_struct(*this_).size
								+ (access_struct(*this_).size - 2)
...
void s_str_sepwith_s_str(s_str *const this_,
		const s_str *const s_str_ptr_for_sepwith) {
	assert(
			this_ != NULL && *this_ != NULL && s_str_ptr_for_sepwith != NULL && *s_str_ptr_for_sepwith != NULL);
	...
	if (access_struct(*this_).size > 1
		...
		char *const slate =
				malloc(
						sizeof(char)
								* (access_struct(*this_).size
										+ (access_struct(*this_).size - 2)
												* (access_struct(*s_str_ptr_for_sepwith).size
														- 1)));
		...
		if (slate != NULL) {
			for (size_t num_iterations = 0, counter = 0;
				...
				slate[counter] = access_struct(*this_).string[num_iterations];
				memcpy(slate + counter + 1,
						access_struct(*s_str_ptr_for_sepwith).string,
						sizeof(char)
								* (access_struct(*s_str_ptr_for_sepwith).size
										- 1));
			}
...
void s_str_replace(const s_str *const this_, const size_t index, size_t length,
		const char c) {
	assert(this_ != NULL && *this_ != NULL);
...
void s_str_replace_all(const s_str *const this_, const char c) {
	...
	if (access_struct(*this_).size > 1) {
		memset(access_struct(*this_).string, c,
				sizeof(char) * (access_struct(*this_).size - 1));
	}
}
...
void s_str_swap(s_str *const this_, s_str *const s_str_ptr_for_swap) {
	assert(
			this_ != NULL && *this_ != NULL && s_str_ptr_for_swap != NULL && *s_str_ptr_for_swap != NULL);
...
void s_str_foreach(const s_str *this_, const size_t index, size_t length,
		void (*const fptr)(char*)) {
...
void s_str_foreach_all(const s_str *this_, void (*const fptr)(char*)) {
	...
	for (size_t counter = 0; counter < access_struct(*this_).size - 1;
			++counter) {
		fptr(access_struct(*this_).string + counter);
	}
}
...
int s_str_compare_c_str(const s_str *const this_, const char *const c_str_ptr) {
	...
	if (access_struct(*this_).size - 1 > strlen(c_str_ptr)) {
		const int compared_val = memcmp(access_struct(*this_).string, c_str_ptr,
				sizeof(char) * strlen(c_str_ptr));
...
int s_str_compare_s_str(const s_str *const this_,
		const s_str *const s_str_ptr_for_compare) {
	assert(
			this_ != NULL && *this_ != NULL && s_str_ptr_for_compare != NULL && *s_str_ptr_for_compare != NULL);

	if (access_struct(*this_).size - 1
			> access_struct(*s_str_ptr_for_compare).size - 1) {
		const int compared_val = memcmp(access_struct(*this_).string,
				access_struct(*s_str_ptr_for_compare).string,
				sizeof(char)
						* (access_struct(*s_str_ptr_for_compare).size - 1));
	...
	} else if (access_struct(*this_).size - 1
			< access_struct(*s_str_ptr_for_compare).size - 1) {
		const int compared_val = memcmp(access_struct(*this_).string,
				access_struct(*s_str_ptr_for_compare).string,
				sizeof(char) * (access_struct(*this_).size - 1));
